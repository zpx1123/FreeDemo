...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ReductionCell *cell = [tableView dequeueReusableCellWithIdentifier:cellId];
    
    if (cell == nil)
    {
        cell = [[[ReductionCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellId] autorelease];
    }
    
    ReductionModel *model = [_appIntroductionMutableArray objectAtIndex:[indexPath row]];
...
- (void)refreshDataCompletion:(void (^)(BOOL finished))completion
{
    ...
    NSString  *urlString = [NSString stringWithFormat:@"http://1000phone.net:8088/app/iAppFree/api/reduce.php?page=%d&number=%d",_currentPage,_numberOfPage];
    ...
    [QFHTTPManager requestWithUrl:urlString Finish:^(NSData *data) {
        ...
        {
            ReductionModel *model = [[ReductionModel alloc] init];
...
{
    ReductionModel    *model = [_appIntroductionMutableArray objectAtIndex:index];
    return model.appId;
}
