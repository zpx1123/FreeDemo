...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell == nil)
    {
        cell = [[[FreeCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellId] autorelease];
    }
    
    FreeModel *model = [_appIntroductionMutableArray objectAtIndex:[indexPath row]];
...
- (void)refreshDataCompletion:(void (^)(BOOL finished))completion
{
    ...
    NSString  *urlString = [NSString stringWithFormat:@"http://1000phone.net:8088/app/iAppFree/api/free.php?page=%d&number=%d",_currentPage,_numberOfPage];
...
{
    FreeModel    *model = [_appIntroductionMutableArray objectAtIndex:index];
    return model.appId;
}
