...
@interface freeRootViewController ()

@end

@implementation freeRootViewController
...
{
    [[freeViewController shareFreeViewController] showSetViewAnimated:YES];
}

- (void) categoryButtonClick:(id)sender
{
    [[freeViewController shareFreeViewController] showCategoryViewAnimated:YES];
}
...
- (void)viewDidLoad
{
    ...
    _freeView = [[FreeView alloc] initWithFrame:self.view.bounds];
    _freeModel = [[FreeModel alloc] init];
    _freeView.dataSource = _freeModel;
    _freeView.delegate = self;
    self.view  = _freeView;
...
{
    NSString *appId = [_freeModel getAppIDWithIndex:[indexPath row]];
    
    _appid = appId;
    
    [[freeViewController shareFreeViewController] showAppDetailViewWithAppID:appId animated:YES];
}
...
{
    //刷新数据
    [_freeModel refreshDataCompletion:^(BOOL finished) {
        
        if (finished)
        {
            [_freeView reloadData];
        }
    }];
}
